...
public class TodoListImplTest {
	...
	public void testJoin() {
		assertEquals(9, todoList.join(completedTodoList).getTasks().size());
	...
	public void testPercentageCompleted() {
		assertEquals(20.00, todoList.percentageCompleted(), 0);
		assertEquals(100.00, completedTodoList.percentageCompleted(), 0);
	}
	...
	public void filterByTags() {
		assertEquals(0, todoList.filter(Criteria.tags(new String[] {"food", "school"}))
				.getTasks().size());
		assertEquals(1, todoList.filter(Criteria.tags(new String[] {"social"}))
				.getTasks().size());
